...
- **Accounts**: Anthropic API key exists (not yet in shell env). Twilio, ElevenLabs, and a Firebase project must be created in Phase 1 — user does signup/payment steps; keys go in `.env.local` / Functions secrets, never in git.
